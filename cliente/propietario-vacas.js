...
(() => {
  ...
  async function cargarVacas() {
    try {
      ...
      if (r.status === 403) {
        ...
        throw new Error('Solo el PROPIETARIO puede ver vacas. ' + (t || ''));
      }
      if (!r.ok) {
        const t = await r.text().catch(()=>'');
        throw new Error('Error al cargar vacas: ' + (t || r.status));
      }
      ...
      console.error(e);
      alert(e.message || 'Error al cargar vacas');
      document.getElementById('vacas-list').innerHTML =
        '<div class="vaca-row" style="opacity:.8">No se pudieron cargar las vacas.</div>';
    }
  }
  ...
  async function eliminarVaca(vacaId) {
    ...
    try {
      ...
      await cargarVacas(); // refresca listado
    } catch (e) {
  ...
  function wireFiltroLocal() {
    ...
    input.addEventListener('input', () => {
      ...
      const vacas = TODAS_VACAS.filter(v => {
        const texto = [v.numero, v.numero_pierna, v.nombre, v.raza, v.status]
